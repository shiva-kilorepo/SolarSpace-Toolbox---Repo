########################################################################
"""SiTE OPTIMIZED SINGLE AXIS TRACKER GRADING ESTIMATE

Revision log
0.0.1 - 8/30/2021 - updated to new template
0.0.2 - 12/14/2022 - Added plane of array slopes, pile checking output, CAD csv output
1.0.0 - 4/2/2022 - Updated parameters for specific outputs to be more clear, simplified output
1.2.0 - 12/9/2022 - Added ability to calculate volume statistics, simplified grading boundary, added ability to export LandXML
1.2.1 - 2/25/2024 - Added checking input protocol and no grading checking
...
class SATGradingEstimate(object):
    def __init__(self):
        self.label = "SiTE Optimized Single Axis Tracker Grading Estimate"
    ...
    def updateParameters(self, parameters):
        ...
        if not parameters[7].altered:
            parameters[7].value = "demGrade"

        if not parameters[8].altered:
            parameters[8].value = "pilesGrade"

        if not parameters[9].altered:
            parameters[9].value = "demGrade_bounds"
        ...
        if not parameters[11].altered:
            parameters[11].value = "Cut"

        if not parameters[12].altered:
            parameters[12].value = "Fill"

        if not parameters[13].altered:
            parameters[13].value = "CutFill_Statistics"
        ...
        if not parameters[15].altered:
            parameters[15].value = "demGrade_LXML.xml"
    ...
    def execute(self, parameters, messages):
        ...
        # Run checking function to ensure the input rows are valid
        SATGradingEstimate.inputCheck(demInput, rowsInput, pilesInput, minReveal, maxReveal, gridRes, mapUnits)
        ...
        # Expand rows east/west by 10 ft (this should be sufficient, but need to test)
...
def xNewTG(pos,x):
    ...
    """
        ...
        arcpy.management.CalculateField(tGroup, "tGroup", "!OBJECTID!", "PYTHON3", None, "LONG")
...
"""
        ...
        codeblock_nsDist = """
def direction(angle, nearDist):
    if (angle > -170) or (angle < 0 and angle < 10) or (angle > 170 and angle < 190):
        return nearDist
    else:
        return ""
    """
...
def yNew(pos,y,ydist):
    ...
    """
        ...
        basePlane_bounds = arcpy.management.MinimumBoundingGeometry(new_points, "basePlane_bounds","RECTANGLE_BY_AREA", "LIST", "PolygonOID","NO_MBG_FIELDS")
            ...
            try:
                ...
                with arcpy.da.SearchCursor(tG, "SHAPE@") as cursor:
                    for row in cursor:
                        ...
                        arcpy.Mosaic_management(outPlanes, planes, "BLEND")
            except Exception as err:
                arcpy.AddMessage(str(err.message))
        ...
        baseplanes = arcpy.management.MosaicToNewRaster(planesAll, workspace, "baseplanes", spatialRef, "32_BIT_FLOAT",gridRes, 1, "BLEND", "FIRST")
        ...
        arcpy.SetProgressor("default", "Calculating the initial grading...")
        ...
        upperLimit = arcpy.sa.Plus(baseplanes, spacing)
        ...
        lowerLimit = arcpy.sa.Minus(baseplanes, spacing)
        ...
        # Calculate reveals 
        grade_trends = arcpy.sa.Minus(demGrade, baseplanes)
        ...
        arcpy.sa.ExtractMultiValuesToPoints(piles_working,[[demInput, "demExist"], [demGrade, "demGrade"], [reveals, "reveal"],[POA, "TOP_elv"]], "BILINEAR")
        ...
        arcpy.management.AlterField(piles_working, "POINT_X", "", "Easting", "DOUBLE", 4, "NULLABLE", "DO_NOT_CLEAR")
        arcpy.management.AlterField(piles_working, "POINT_Y", "", "Northing", "DOUBLE", 4, "NULLABLE", "DO_NOT_CLEAR")
        arcpy.management.AlterField(piles_working, "demExist", "", "Existing Elevation", "FLOAT", 4, "NULLABLE","DO_NOT_CLEAR")
        arcpy.management.AlterField(piles_working, "demGrade", "", "Graded Elevation", "FLOAT", 4, "NULLABLE","DO_NOT_CLEAR")
        arcpy.management.AlterField(piles_working, "reveal", "", "Reveal", "FLOAT", 4, "NULLABLE", "DO_NOT_CLEAR")
        arcpy.management.AlterField(piles_working, "TOP_elv", "", "Top of Pile Elevation", "FLOAT", 4, "NULLABLE","DO_NOT_CLEAR")
        arcpy.management.AlterField(piles_working, "nsSlopePercPOA", "", "North-South POA Slope", "DOUBLE", 4,"NULLABLE", "DO_NOT_CLEAR")
        arcpy.management.AlterField(piles_working, "cutFill", "", "Cut Fill", "DOUBLE", 4, "NULLABLE", "DO_NOT_CLEAR")
        ...
        arcpy.management.Delete("baseplanes")
        ...
        # If no grading is required, exit the script
        if str(cutFillMAX) == '0' and str(cutFillMIN) == '0':
            arcpy.AddMessage("No grading required for site")
            
            # exit script
            arcpy.AddMessage("No grading required for site; no grading output created")
            #raise arcpy.ExecuteError("No grading prescribed")
            #sys.exit(0)
            return
            ...
            # Define the distance between points for the center row
            pointDist = str(round(gridRes * 4)) + str(" Feet")
        ...
        arcpy.management.Delete(basePlane_bounds)
